...
def AddDerivedFeatures(df: pl.DataFrame) -> pl.DataFrame:
...
def GenerateLatentDifficulty(df: pl.DataFrame, rng: np.random.Generator) -> np.ndarray:
    ...
    case_type_effect = StableCategoryEffect(df["case_type"].to_list(), scale=0.35)
    court_effect = StableCategoryEffect(df["court"].to_list(), scale=0.25)
    jurisdiction_effect = StableCategoryEffect(df["jurisdiction"].to_list(), scale=0.15)
    stage_effect = StableCategoryEffect(df["stage"].to_list(), scale=0.20)
    noise = rng.normal(loc=0.0, scale=0.40, size=df.height)
    latent = (
        0.30 * word_signal
...
def GenerateComplexityScore(latent: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    """
    Continuous complexity score:

        0 <= complexity_score <= 100
    """
    noise = rng.normal(loc=0.0, scale=0.30, size=len(latent))
    raw = latent + noise
    complexity = Sigmoid(raw * 1.35) * 100.0

    return np.clip(complexity, 0.0, 100.0).astype(np.float64)


def GenerateAdjournment(
    df: pl.DataFrame, latent: np.ndarray, rng: np.random.Generator
) -> np.ndarray:
    case_type_effect = StableCategoryEffect(df["case_type"].to_list(), scale=0.40)
    court_effect = StableCategoryEffect(df["court"].to_list(), scale=0.30)
    noise = rng.normal(loc=0.0, scale=0.35, size=df.height)

    logit = (
        -0.20 + 0.75 * latent + 0.20 * case_type_effect + 0.15 * court_effect + noise
    )
    probability = Sigmoid(logit)
    adjourned = rng.binomial(n=1, p=probability, size=df.height)
...
def GenerateDuration(
    ...
    section_signal = Standardize(np.log1p(section_count))
    judge_signal = Standardize(np.log1p(judge_count))
    case_type_effect = StableCategoryEffect(df["case_type"].to_list(), scale=50.0)
    court_effect = StableCategoryEffect(df["court"].to_list(), scale=35.0)
    noise = rng.normal(loc=0.0, scale=85.0, size=df.height)
    duration = (
        240.0
        + 115.0 * latent
        + 35.0 * section_signal
    ...
    return np.clip(duration, 1.0, 3000.0).astype(np.float64)


def BuildTrainingDataset(df: pl.DataFrame) -> pl.DataFrame:
    rng = np.random.default_rng(RANDOM_STATE)
    df = NormalizeCategoricalColumns(df)
    df = AddDerivedFeatures(df)

    latent = GenerateLatentDifficulty(df, rng)
    complexity_score = GenerateComplexityScore(latent, rng)
    adjourned = GenerateAdjournment(df, latent, rng)
    duration_days = GenerateDuration(df, latent, adjourned, rng)

    df = df.with_columns(
        pl.Series("complexity_score", complexity_score, dtype=pl.Float64),
        pl.Series("adjourned", adjourned, dtype=pl.Int8),
        pl.Series("duration_days", duration_days, dtype=pl.Float64),
    )

    return df.select(FEATURE_COLUMNS + TARGET_COLUMNS)


def WriteTrainingDataset(df: pl.DataFrame) -> None:
    PROCESSED_DATA_DIR.mkdir(parents=True, exist_ok=True)

    df.write_parquet(TRAINING_PARQUET, compression="zstd")
    df.write_csv(TRAINING_CSV)


def PrintReport(df: pl.DataFrame) -> None:
    print()
    print("=" * 72)
